...
-- Fired when the "Kindle Fetch" dispatcher action is triggered, e.g. via a
-- gesture assigned in Settings > Taps and gestures > Assign action.
...
function KindleFetch:performSearch()
    ...
    -- make sure we're online, turning WiFi on automatically if it isn't
    -- already, then run the actual search once connected
    NetworkMgr:runWhenOnline(function()
        this:doSearch(query)
    end)
end

function KindleFetch:doSearch(query)
