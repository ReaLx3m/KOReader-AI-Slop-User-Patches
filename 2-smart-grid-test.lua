...
Installation:
  Copy to:  koreader/patches/2-smart-grid.lua
...
if FileChooser._smart_grid_patched then return end
FileChooser._smart_grid_patched = true
...
function FileManagerMenu:setUpdateItemTable()
    ...
    -- Append the Smart Grid sub-entry (guard against duplicate injection).
    local already = false
    for _, item in ipairs(self.menu_items.ai_slop_settings.sub_item_table) do
        if item._smart_grid_entry then already = true; break end
    end
    if not already then
        table.insert(self.menu_items.ai_slop_settings.sub_item_table, {
            _smart_grid_entry = true,
            text = "Smart Grid",
            sub_item_table_func = function()
                return buildMenu(fc)
            end,
...
logger.info("smart-grid: patch applied")
