...
local function getPageCount(filepath)
    ...
    -- highest priority: explicit page count in filename e.g. "Book Title p(234).epub" or "Book Title p234.epub"
...
local function paintProgressBadge(bb, cover_x, cover_y, cover_w, cover_h,
    ...
    -- nil percent means book never opened; skip badge entirely
    if percent == nil and not is_finished then return end

    ot        = ot        or 2
    outline_r = outline_r or ot

    -- Badge dimensions: Ry controls height, Rx controls width
    local Ry_full = math.max(16, math.floor(math.min(cover_w, cover_h) * 0.143))
    local Ry = math.floor(Ry_full * 0.96)   -- badge is 10% shorter
    local Rx = math.floor(Ry_full * 1.375)  -- width unchanged (based on full Ry)

    -- Inner corner flush against the inside face of the outline
    local ax = cover_x + cover_w - 1 - ot
    local ay = cover_y + cover_h - 1 - ot

    -- Badge extents (top uses reduced Ry)
    local bx = ax - Rx + 1
    local bt = ay - Ry + 1
    -- Text anchor uses the original full height so text doesn't shift down
    local bt_text = ay - Ry_full + 1

    -- Badge rounded top-left corner radius (based on full Ry so it doesn't shrink)
    local rc       = math.max(4, math.floor(math.min(Rx, Ry_full) * 0.20))
    ...
    local badge_fs = math.max(6, math.floor(math.min(Rx, Ry_full) * 0.52 / getDPIScale()))
    local tw, ts
    for _ = 1, 10 do
        tw = TextWidget:new{
            text    = text_str,
    ...
    -- center text vertically using original full height (text doesn't move when badge shrinks)
    local badge_h = ay - bt_text
    local badge_mid_y = math.floor(bt_text + badge_h * 0.58)
