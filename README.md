...
When you make changes to the Folder and/or File Modes, it might appear that the changes arent applied. You need to either press Home or access a different folder from your current one, for the view to refresh and show correctly from there on. Already have a version that fixes that and applies the changes on the fly, but is still not tested. Maybe ill release it maybe not, not a big annoyance as is i think. If you do want that version to try out, ask for it over at "Issues" tab. 
